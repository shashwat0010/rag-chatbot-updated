...
from services.guardrails import validate_answer_grounding, is_greeting_or_meta, check_query_safety, QueryAnalysisResult, INSUFFICIENT_EVIDENCE_MESSAGE
...
class RAGPipeline:
    ...
    async def run_stream(self, query: str, max_papers: Optional[int] = None, risk_level: str = "LOW", analysis: Optional[QueryAnalysisResult] = None):
        settings = get_settings()
        ...
        if is_greeting_or_meta(query):
            async for token in self._get_llm().generate_stream(query, []):
        ...
        # 3. Handle zero search results fallback
        if not papers:
            yield f"event: token\ndata: {json.dumps(INSUFFICIENT_EVIDENCE_MESSAGE)}\n\n"
            metadata = {
                "citations": [],
                "confidence_note": (
                    "No relevant papers were retrieved from PubMed for this query. "
        ...
        if not retrieval_is_sufficient(chunks, papers, use_local=False):
            ...
            yield f"event: token\ndata: {json.dumps(INSUFFICIENT_EVIDENCE_MESSAGE)}\n\n"
            metadata = {
                "citations": [c.model_dump() if hasattr(c, "model_dump") else c.dict() if hasattr(c, "dict") else c for c in self._chunks_to_citations(evidence_chunks[:3])],
                "confidence_note": (
                    "Retrieved literature had very low semantic match to your question. "
                    "Try rephrasing with medical subject terms (drug class, condition, outcome)."
                ),
        ...
        # 6. Stream the answer from candidate chunks
        full_answer_buffer = []
        async for token in self._get_llm().generate_stream(normalized_query, evidence_chunks):
            full_answer_buffer.append(token)
            yield f"event: token\ndata: {json.dumps(token)}\n\n"

        full_answer = "".join(full_answer_buffer)

        # 7. Post-generation: Resolve citations from markdown text
        citation_matches = re.findall(r'\[(\d+)\]', full_answer)
        cited_indices = sorted(list(set(int(idx) for idx in citation_matches)))
        ...
        insufficient = INSUFFICIENT_EVIDENCE_MESSAGE.lower() in full_answer.lower()
        ...
        if risk_level == "HIGH":
            confidence_note = "High-risk query detected. Confidence strictly reduced. " + confidence_note
            disclaimer_suffix = "\n\n**Important Safety Notice:** Current evidence does not support replacing evidence-based treatment with unverified alternatives. Supportive approaches may help symptom management, but treatment decisions should be made with a licensed clinician or specialist."
            yield f"event: token\ndata: {json.dumps(disclaimer_suffix)}\n\n"
            final_answer += disclaimer_suffix
